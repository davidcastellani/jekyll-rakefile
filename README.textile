...
A Rakefile to simplify previewing and deploying "jekyll":http://jekyllrb.com websites behind Cloudflare.
...
2. If you use ```rake preview``` it will now include posts in _drafts, which must now have a date so that jekyll knows where to put the draft posts in the post order.
